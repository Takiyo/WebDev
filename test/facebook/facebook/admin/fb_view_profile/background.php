...
?>
<?php 
	$que_v_user_info=mysqli_query("select * from users where user_id=$v_user_id");
	...
	$que_view_user_profile_pic=mysqli_query("select * from user_profile_pic where user_id=$v_user_id");
	$user_profile_pic_data=mysqli_fetch_array($que_view_user_profile_pic);
	$profile_img=$user_profile_pic_data[2];


	$que_user_cover_pic=mysqli_query("select * from user_cover_pic where user_id=$v_user_id");
	...
	$que_post_img=mysqli_query("select * from user_post where user_id=$v_user_id and post_pic!='' order by post_id desc");
...
{
	mysqli_query("update user_status set status='Offline' where user_id=$v_user_id");
}
if(isset($_POST['delete_user']))
{
		$delete_id=intval($_POST['delete_id']);
		mysqli_query("delete from users where user_id=$delete_id;");
...
if(isset($_POST['warning_user']))
{
		...
		mysqli_query("insert into user_warning values($warning_id,'$warning_txt')");
}
...
if(isset($_POST['file']) && ($_POST['file']=='Upload'))
{
    	...
    	mysqli_query("update user_profile_pic set image='$img_name' where user_id=$v_user_id;");
...
if(isset($_POST['file1']) && ($_POST['file1']=='Upload'))
{
    	...
    	mysqli_query("insert into user_cover_pic(user_id,image) values('$v_user_id','$img_name');");
		header("location:view_profile.php?id=".$v_user_id."");
}
...
if(isset($_POST['file2']) && ($_POST['file2']=='Upload'))
{
		...
		mysqli_query("update user_cover_pic set image='$img_name' where user_id=$v_user_id;");
		header("location:view_profile.php?id=".$v_user_id."");
}
?>
<html>
...
<?php
	 $query_online=mysqli_query("select * from user_status where status='Online'");
...
<?php
	 while($online_data=mysqli_fetch_array($query_online))
	 {
		...
		$query_online_user_data=mysqli_query("select * from users where user_id=$online_user_id");
		$query_online_user_pic=mysqli_query("select * from user_profile_pic where user_id=$online_user_id");
