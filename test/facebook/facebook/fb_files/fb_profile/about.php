...
<?php
	if(isset($_POST['work_sub']))
	{
		$u_job=$_POST['job'];
		$u_edu=$_POST['edu'];
		mysqli_query("update user_info set job='$u_job',school_or_collage='$u_edu' where user_id=$userid;");
	}
	...
	if(isset($_POST['leving_sub']))
	{
		...
		mysqli_query("update user_info set  	current_city='$u_city',hometown='$u_hometown' where user_id=$userid;");
	}
	...
	if(isset($_POST['basic_sub']))
	{
		if($_POST['day']=='Day:' && $_POST['month']=='Month:' && $_POST['year']=='Year:')
		{
			...
			mysqli_query("update user_info set relationship_status='$u_relationship' where user_id=$userid;");
		}
		else
		{
	...
	if(isset($_POST['contact_sub']))
	{
		...
		$u_web=$_POST['web'];
		$u_fb_id=$_POST['fbid'];
		mysqli_query("update user_info set mobile_no='$u_m_no',mobile_no_priority='$u_priority',website='$u_web',Facebook_ID='$u_fb_id' where user_id=$userid;");
	}
	
		include("background.php");
		
		$user_info_query=mysqli_query("select * from user_info where user_id=$userid");
...
<?php
	$que_post_img=mysqli_query("select * from user_post where user_id=$userid and post_pic!='' order by post_id desc");
...
<div style="position:absolute;left:17%; top:139%; height:0.09%; width:66%; background:#CCC; z-index:1">
 ...
 <!--Basic Information--> 
 <?php
	$user_data_query=mysqli_query("select * from users where Email='$user'");
